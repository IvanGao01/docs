...
| tagsExtra            | Map type, configured with additional tags, as an additional column for each row of data, and imported into CnosDB. See the instructions below for the format details.详细格式见下方说明。                                                                      | No   | -                                    |
| fieldsExtra          | Map type to configure which tables and columns of CnosDB the data from some columns of reader is output to. Only works if format is `opentsdb`. See the instructions below for the format details.仅当 format 为 `opentsdb` 时生效。详细格式见下方说明。              | No   | *                                    |
